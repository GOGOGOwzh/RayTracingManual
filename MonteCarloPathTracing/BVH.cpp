...
std::shared_ptr<BVH_Node> BVH::RecursiveBuild(std::vector<Object*> vecObj)
{
	...
	if (vecObj.size() == 1)
	{
		...
		node->area = node->pLeftNode->area + node->pRightNode->area;
		return node;
	}
	else {
		BoundingBox box;
...
void BVH::Sample(Vector3f& pos, Vector3f& normal, float& pdf)
{
	float p = std::sqrt(Get_random_float()) * m_rootNode->area;
	getSample(m_rootNode, p, pos,normal, pdf);
	pdf /= m_rootNode->area;
}

void BVH::getSample(std::shared_ptr<BVH_Node> node, float p, Vector3f& pos, Vector3f& normal, float& pdf)
{
	if (node->pLeftNode == nullptr || node->pRightNode == nullptr) {
		node->pObj->Sample(pos,normal, pdf);
		pdf *= node->area;
		return;
	}
	if (p < node->pLeftNode->area) getSample(node->pLeftNode, p, pos,normal, pdf);
	else getSample(node->pRightNode, p - node->pLeftNode->area, pos,normal,pdf);
}
